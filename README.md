...
7. Update the settings in the [manifest.yml](https://github.com/hackclub/toriel/blob/main/manifest.yml)
   - Change the slash command and event endpoints by replacing `https://to.underpass.clb.li` with your ngrok URL
...
14. Do a global find and replace in the codebase to update `channels.arv` to `channels.name-of-your-private-channel`
    - As `{channels.arv}` refers to `#in-the-cave`, we need to replace it with the private channel (ex. `{channels.msw-test-cave}`) that you created for your bot
