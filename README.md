...
14. Do a global find and replace in the codebase to update `channels.arv-tor` to `channels.name-of-your-private-channel`
    - As `{channels.arv-tor}` refers to `#in-the-cave`, we need to replace it with the private channel (ex. `{channels.msw-test-cave}`) that you created for your bot
